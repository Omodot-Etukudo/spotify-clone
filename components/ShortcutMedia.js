...
function ShortcutMedia({picture, title, isPlaying, progress, isNew}) {
    return (
        <div title={title} className=" w-96 rounded-md group h-16 flex space-x-4 pr-0 bg-gray-800 hover:bg-gray-700 transition-call duration-500 cursor-pointer shadow-2xl drop-shadow-2xl">
            ...
            <div className="flex justify-between items-center w-9/12">
                <div className="my-auto flex flex-col space-y-2 justify-start items-start">
                    <div className="flex flex-row justify-center items-center gap-1.5">
                        <div className={`${isNew ? "w-1.5 h-1.5 bg-blue-600 rounded-full block":"hidden"}`}></div><h3 className="font-semibold text-gray-100 text-xs w-56">{title}</h3>
                    </div>
