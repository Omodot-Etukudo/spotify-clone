                    ...
                    <div key={content.id} className="flex flex-col space-y-6 h-screen lg:h-full lg:w-full lg:pl-56 lg:pr-6 lg:max-w-full max-w-full min-w-full py-20 lg:pt-16 lg:pb-6 lg:mb-10 px-3 lg:dark:bg-main-bg bg-white">
                       <div className="w-full py-1 flex justify-start items-end space-x-6">
                            <div className=" w-48 h-48 flex justify-center items-center">
                                <Image alt="Song Name"  placeholder="blur" loading="lazy" blurDataURL width={200} height={200} className="w-full h-full"  src={`${BASE_URL}${content.cover}` || `${BASE_URL2}${content.cover}` || `${BASE_URL3}${content.cover}`} />
                            </div>
                            <div className="flex flex-col space-y-8 justify-end dark:text-white text-main-bg" >
                                <div className="flex flex-col space-y-2">
                                    ...
                                    {/* Check for name length to define font size to be used for title */}
                                ...
                                <table className="w-full pl-4">
                                    ...
                                    <tbody>
                                        {content.data.map((song=>{
                                            return(
                                                <DefaultSongComponent position={content.data.indexOf(song)+1} key={song.id} isLiked={song.isLiked} artist={content.creator} picture={content.cover} title={song.name} isAlbum/>
                                            )
