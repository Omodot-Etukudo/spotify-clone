...
function Mediacard({picture, title, artist, isplaylist, isPodcast}) {
    
    return (
        <Link passHref  href="/album/jsdk">
            <div className="group h-54">
                <div className=" cursor-pointer w-32 lg:w-38 lg:bg-card-bg lg:shadow-xl bg-opacity-50 lg:px-4 pt-4 pb-5 flex flex-col rounded lg:group-hover:bg-card-hover transition-colors duration-500">
                    ...
                    <div className="title-text">
                        <h3 className="text-xs lg:text-xs mt-2 lg:mt-4 font-semibold text-white lg:text-left text-center truncate">{title}</h3>
                    </div>
                
                    <div className="flex lg:text-left text-center"><h3 className=" text-xxs mt-1 truncate lg:text-left lg:mx-0 mx-auto">{artist}</h3></div>
