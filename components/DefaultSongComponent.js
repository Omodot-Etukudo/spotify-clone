...
function DefaultSongComponent({title, duration, isPlaying, isLiked, isAlbum, picture, artist, position}) {
    return (
            ...
            <tr className=" cursor-default rounded-md group dark:hover:bg-card-hover dark:bg-main-bg bg-white hover:bg-gray-100 transition-all duration-150">
                <td className="py-2 px-2 w-8 font-light text-xxs"><p className="block group-hover:hidden">{position}</p> <PlayIcon className="w-4 h-4 dark:text-gray-100 text-main-bg hidden group-hover:block" /> </td>
                <td className="py-2">
                    <div className="flex space-x-3 items-center justify-start">
                        ...
                        <div className="flex flex-col space-y-1"><p className="text-xs dark:text-gray-100 text-main-bg">{title}</p><p className="text-xxs text-gray-400">{artist}</p></div>
                    </div>
                    </td>  
                <td className={`${isAlbum?"hidden":"py-2 text-xxs dark:text-gray-400 text-gray-700"}`} >{title}</td>
                <td className={`${isAlbum?"hidden":"py-2 text-xxs dark:text-gray-400 text-gray-700"}`} >{title}</td>
